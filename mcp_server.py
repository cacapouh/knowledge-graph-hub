...
ツール一覧:
  ...
  - list_skills               : Skill 一覧
  - get_skill                 : Skill 詳細
  - create_skill              : Skill 新規作成 (prompt + MCPs)
  - update_skill              : Skill 部分更新
  - delete_skill              : Skill 削除
...
@mcp.tool()
def list_skills() -> str:
    """Skill の一覧を取得する。Skill は再利用可能なプロンプト + MCPs のバンドル。"""
    with _client() as c:
        r = c.get("/skills")
        r.raise_for_status()
    return r.text
...
@mcp.tool()
def get_skill(skill_id: int) -> str:
    """指定 ID の Skill 詳細を取得する。"""
    with _client() as c:
        r = c.get(f"/skills/{skill_id}")
        r.raise_for_status()
    return r.text
...
def create_skill(
    ...
    mcps は MCP server 設定の配列 (Claude Code .mcp.json 互換形式):
      stdio: {"name": "...", "type": "stdio", "command": "...", "args": [...], "env": {...}}
      http : {"name": "...", "type": "http",  "url": "https://...", "headers": {...}}
      sse  : {"name": "...", "type": "sse",   "url": "https://..."}
    """
    body: dict = {
        "name": name,
        "prompt": prompt,
...
def update_skill(
    ...
    """Skill のフィールドを部分更新する。指定したフィールドのみ書き換わる。"""
    body: dict = {}
    if name is not None:
        body["name"] = name
    if description is not None:
    ...
    if mcps is not None:
        body["mcps"] = mcps
    with _client() as c:
        r = c.patch(f"/skills/{skill_id}", json=body)
        r.raise_for_status()
    return r.text


@mcp.tool()
def delete_skill(skill_id: int) -> str:
    """Skill を削除する。skill 型のプロパティで参照されている場合、参照側は orphan になる点に注意。"""
    with _client() as c:
        r = c.delete(f"/skills/{skill_id}")
        r.raise_for_status()
