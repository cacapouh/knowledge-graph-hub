...
ツール一覧:
  ...
  - list_views                : Saved View 一覧
  - get_view                  : Saved View 詳細 (+ Graph View URL)
  - create_view               : Saved View 新規作成 (conditions[] で条件指定)
  - update_view               : Saved View 更新
  - delete_view               : Saved View 削除
"""
...
def _view_url(view_id: int) -> str:
    """Graph View URL that opens the saved view (applies all conditions)."""
    return f"{FRONTEND_BASE}/graph?viewId={view_id}"


@mcp.tool()
def list_views() -> str:
    """Saved View 一覧を取得する。各 View は conditions[] (OR 結合) でノード/エッジ集合を定義する。"""
    with _client() as c:
        r = c.get("/views")
        r.raise_for_status()
    return r.text
...
@mcp.tool()
def get_view(view_id: int) -> str:
    """Saved View 詳細を取得する。"""
    with _client() as c:
        r = c.get(f"/views/{view_id}")
        r.raise_for_status()
    return f"{r.text}\n\n📊 Graph View: {_view_url(view_id)}"
...
def create_view(
    ...
    可視ノード/エッジになる。各要素は以下のいずれか:
      ...
      {"kind": "type_filter",
       ...
       "link_type_ids":   [int, ...]}   # 表示するエッジ種別 (空なら制約なし)

      {"kind": "neighborhood_of_type",
       "object_type_id": int,           # 起点となるノード種別 (全インスタンス)
    ...
    with _client() as c:
        r = c.post("/views", json=body)
        r.raise_for_status()
    data = r.json()
    return f"{r.text}\n\nView #{data['id']} created\n📊 Graph View: {_view_url(data['id'])}"
...
def update_view(
    ...
    """Saved View を更新する。conditions のスキーマは create_view を参照。"""
    body: dict = {}
    if name is not None:
        body["name"] = name
    if description is not None:
    ...
    with _client() as c:
        r = c.put(f"/views/{view_id}", json=body)
        r.raise_for_status()
    return f"{r.text}\n\n📊 Graph View: {_view_url(view_id)}"


@mcp.tool()
def delete_view(view_id: int) -> str:
    """Saved View を削除する。"""
    with _client() as c:
        r = c.delete(f"/views/{view_id}")
        r.raise_for_status()
